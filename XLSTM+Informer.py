...
def cal_eval(y_real, y_pred):
    ...
    return df_eval
df = pd.read_csv('D:\pythonProject\\xinformer\XLSTM+informer\XLSTM+informer\inf\data\ETTm2.csv') # 将你所需要的数据放到data文件夹，然后只要改成你自己的文件名
...
window = 5 # 模型输入序列长度 这里只能是 12345 的这种次方倍数 这个改的话 model层的241行也得改
length_size = 10  # 预测结果的序列长度
batch_size = 64
