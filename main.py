...
data = pd.read_csv('D:\pythonProject\\xinformer\XLSTM+informer\XLSTM+informer\inf\data\\test_data_单变量.csv', usecols=[5], engine='python')
print(data)
dataset=data
...
def create_dataset(dataset, seq_len=19):
    dataX, dataY = [], []
    for i in range(len(dataset) - seq_len):
        a = dataset[i:(i + seq_len - 1)]
        dataX.append(a)
        dataY.append(dataset[i + seq_len - 4])
...
for i in final_dict:
    print('MAE:'f"{i} : {mean_absolute_error(actual, final_dict[i])}")
    print('MSE:'f"{i} : {mean_squared_error(actual, final_dict[i])}")
    print('R2:'f"{i} : {r2_score(actual, final_dict[i])}")
